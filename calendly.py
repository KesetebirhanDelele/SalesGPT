...
def list_available_event_type_uuids():
    ...
    url = 'https://calendly.com/kebededegefu916' # 'https://api.calendly.com/event_types'
...
def generate_calendly_invitation_link(query):
    '''Generate a Calendly invitation link based on the query string'''
    ...
    # Full URI for the event type (not just the UUID)
    owner_uri = f"https://api.calendly.com/event_types/{event_type_uuid}"
    
    url = 'https://api.calendly.com/scheduling_links'
    payload = {
        "max_event_count": 1,
        "owner": owner_uri,  # Full URI for the event type
    ...
    if response.status_code == 201:
        data = response.json()
        return f"Here is your appointment link: {data['resource']['booking_url']}"
    else:
