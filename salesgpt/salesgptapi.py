...
class SalesGPTAPI:
    def __init__(
        ...
        self.openai_api_key = os.getenv('OPENAI_API_KEY')  # Load from environment
    ...
    def initialize_agent(self):
        ...
        # Comment out the verbose log for loading config
            # if self.verbose:
            #     print(f"Loaded agent config: {config}")
        else:
        # Comment out the default config message    
            # print("Default agent config in use")
            pass

        if self.use_tools:
        # Comment out the tools usage log
            # print("USING TOOLS")
            config.update(
                {
                    "use_tools": True,
                    "product_catalog": self.product_catalog,
                    "salesperson_name": "Jakie"
                    if not self.config_path
                    else config.get("salesperson_name", "Jakie"),
                }
            )

        sales_agent = SalesGPT.from_llm(self.llm, **config)
    # Comment out the tools usage check log
    ...
    async def do(self, human_input=None):
        ...
        # Comment out the max turns reached message
        ...
        # Comment out the AI log printing
        # if self.verbose:
        #     print("=" * 10)
        ...
        # Comment out the conversation end detection log    
    ...
    # Comment out intermediate steps handling and tool logs
        ...
        if (
            ...
            # Comment out the action processing log
        ...
        # Print the chatbot response (this is the final response that should be displayed)
        print(reply)
        payload = {
